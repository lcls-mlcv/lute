...
class PlotGenericSolventParameters(ThirdPartyParameters):
    """Parameters for plotting generic solvent scattering data.

    This task plots the generic solvent scattering data from input h5 file, produces output
    ...
    offset_idx: int = Field(
        0,
        description="Offset index to start subsampling from. Default is 0.",
        flag_type="--",
    ...
    scan_max_nbins: Optional[int] = Field(
        ...
        flag_type="--",
        rename_param="scan-max-nbins",
    )

    alpha: float = Field(
        0.25,
        description="Alpha for the plot. Default is 0.25.",
        flag_type="--",
    ...
    def validate_output_dir(cls, output_dir: str, values: Dict[str, Any]):
        ...
        if output_dir == "":
            ...
            hutch: str = exp[:3]
            output_dir = f"/sdf/data/lcls/ds/{hutch}/{exp}/stats/summary/GenericSolventScattering/{run:04d}"
        import os
    ...
    def validate_output_png(cls, output_png: str, values: Dict[str, Any]):
        if output_png == "":
            run: int = int(values["lute_config"].run)
            output_png = f"run{run:04d}_generic_solvent_scattering.png"
        return output_png
